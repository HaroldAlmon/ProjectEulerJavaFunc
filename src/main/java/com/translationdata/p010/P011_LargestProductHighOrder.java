...
interface CellProduct {
	int apply(int row, int column, int length);
}
...
public class P011_LargestProductHighOrder {
	...
	private final CellProduct fallingDiagonalProductLambda = (row, col, productLength) -> {
		return fallingDiagonalProductImpl(row, col, productLength, 1);
	};
	...
	private final CellProduct risingDiagonalProductLambda = (row, col, productLength) -> {
		return risingDiagonalProductImpl(row, col, productLength, 1, productLength);	
	};
	...
	private final CellProduct rowPoductLambda = (row, col, productLength) -> {
	  return rowProductImpl(row, col, productLength, 1);
	};
	...
	private int matrixCellProduct(int[][] matrix, 
			CellProduct calcProductLambda, 
            int rowLen, 
            int columnLen) {
		return IntStream.range(0, matrix.length - 1 - rowLen)
			.map(row -> calculateCellProduct(row, columnLen, matrix, calcProductLambda))
			.max()
			.getAsInt();
	}
	...
	private int calculateCellProduct(int row, 
		...
		return IntStream.range(0, matrix[0].length - (productLen + 1))
			.map( col -> calcProductLambda.apply( row, col, productLen) )
			.max()
			.getAsInt();
	}
